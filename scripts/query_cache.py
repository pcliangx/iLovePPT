...
本 helper 把历史成功 query 沉淀到 `scripts/.cache/external_query_cache.jsonl`,
让后续相似页直接 reuse。

Storage:
    scripts/.cache/external_query_cache.jsonl —— 每行 1 JSON record:
    {
...
try:
    ...
    print(
        "ERROR: rapidfuzz 不可用 — pip install rapidfuzz",
        file=sys.stderr,
    )
    sys.exit(2)

# cache 文件位置:跟脚本同目录的 .cache/(scripts/.cache/external_query_cache.jsonl · gitignored)
CACHE_PATH = Path(__file__).resolve().parent / ".cache" / "external_query_cache.jsonl"
