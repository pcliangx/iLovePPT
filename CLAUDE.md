...
| `iloveppt-audience` | **Step 0.0 spot-check**(P2-3.3 后并入,placeholder grep / chart source / 5+ PNG 破损 detect / red_line grep)+ 模拟目标受众读 deck 评分(9 分硬阈值);反馈三类分流(needs_author_rewrite / needs_visual_redo / needs_theme_fix);**P3-5 评分完 append `library/_rag/feedback.jsonl` 反馈给 RAG** | Task | opus |
