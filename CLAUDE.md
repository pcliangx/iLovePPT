...
iLovePPT 是一个 **agent team 帮你写 PPT 的开源工具**。clone 本仓库后,在仓库根目录跟 Claude Code 说"做 PPT",**5 个 agent**(brainstorm → author → critic[cd 合审] → iloveppt-builder → audience)接力把你的想法变成完整 `.pptx`。 pipeline 7 步:brainstorm(自带 brief self-audit)→ author Stage C → author Stage D → critic stage=cd → builder → audience(自带 Step 0 spot-check)→ 用户 OK。

仓库本身就是产品:

- `.claude/agents/` —— 5 个 agent 定义(5 主流水线),Claude Code 启动时自动加载
...
### Agent 流水线(Hybrid:1 team + 5 subagent + 1 Haiku helper)
...
- **Phase B (subagent 模式)**:author / critic / iloveppt-builder / audience 用 `Task` 工具调用,每次跑完 return yaml(主线程 parse 决定下一步)。
- **Helper agents**:`iloveppt-yaml-fixer` 用 Haiku 跑结构性活,主流水线不直接派,主线程在工程错误恢复路径上 dispatch(详见下方 § Haiku helper agents)。

**模型(主流水线)**:5 个主流水线 agent 全用 **opus**(team 跨阶段一致性 + 整条流水线的深度判断 / 多职责 / 多轮对话 / 视觉认知都需要高质量推理)。
...
`iloveppt-yaml-fixer` 是**辅助 agent**,主流水线 5 agent 链路**不直接派**。主线程在**工程错误恢复**场景 dispatch:agent return yaml malformed(`yaml.safe_load` 异常) → `Task(iloveppt-yaml-fixer, args={yaml_path, failure_report})` → 修完 → 主线程重 parse,失败 → 整 Task 重派原 agent。
...
**为什么 Haiku 够用**:任务范围窄、规则化、输出 schema 严格 — 不需要创造性推理;但仍要 fail-loud,Haiku 4.5 跟得上。**主流水线 5 agent 仍全 Opus**。
...
> ⚠️ **Phase 0 部分清理**:本节以下含历史 RAG / tier1 / extractor / placeholder_map / SQLite WAL 相关不变量,随 Phase 0 切除已**失效**(search.sh / _rag / pptx-templates / extractor 全删)。保留作历史参考,待后续 PR 清理。

## 关键不变量

- **中文字体必须通过 lxml 写 `<a:ea>` + `<a:cs>`**(`helpers/__init__.py:set_font`)。python-pptx 的默认 `font.name` 只写 `<a:latin>`,中文文字跨平台会 fallback 成丑字体。**这是 #1 的产物破损源**。读侧全量校验:`scripts/audit_pptx.py <pptx> --sections fonts`(builder Step 2.9 gate · ERROR = 经典 latin-only bug)
...
| `audit_pptx.py` | 读侧 .pptx 机械审计 · EA 字体 gate(builder Step 2.9)+ shapes/hyperlinks/embedded/MSIP 清单 |
