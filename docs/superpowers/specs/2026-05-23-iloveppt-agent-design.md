...
- **Phase 1 步骤**:读 workflow.md、content-writing.md 的「deck 级论证结构」、diagram-planning.md;设计行动式标题 + Minto 论证弧;过一遍 ghost deck test;规划图层;**输出大纲 + 图层计划 + 提议页数,等用户批准**。
...
- `sections[]`:每节 `{标题, action_title, 意图, 建议 layout, 是否配图}`
