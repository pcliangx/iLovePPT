...
3. **iloveppt-critic** —— 在 outline 和 content 两个 checkpoint 各做一次评审(14 项 checklist + 5 维度判断性评审)
4. **iloveppt-builder** —— 接收审过的 content.md → Step 0-3 构建 `.pptx` + 17 项机械视觉自检 → **Step 4 主动加视觉**(iconify 图标 / Unsplash hero / brand assets)一气呵成
5. **iloveppt-audience** —— 模拟目标受众读 deck 评分(9 分硬阈值,不过反馈到 author / iloveppt-builder mode=visual_redo / theme)
...
> 系统内部用 **Hybrid 架构**:brainstorm 跟你聊天的阶段(Phase A)用持续 team window,延迟低对话流畅;brief 批准后(Phase B)流水线交给 subagent 接力(author → critic → iloveppt-builder → audience)。对你的使用体验透明无差异,只是底层运行更轻量。
...
| 审 content.md(可直接编辑文件) | critic 评审 → author 等批准;批准后 iloveppt-builder(build + 视觉一气呵成) → audience |
| 收 .pptx + audience 评分 + 看 deck_plan_edits + visual_edits 清单 | iloveppt-builder(机械 build + Step 4 视觉)后,audience ≥ 9 才 ok |

> **核心原则——一图胜千文。** 凡涉及结构、流程、关系、数据对比,author 会主动用 draw.io / matplotlib 画图嵌进 markdown,而不是堆文字 bullet。

> **为什么拆 5 agent?** 每个角色窄、可复用、可独立 test;主线程退化为 thin dispatcher,不被 PPT 任务污染;critic / iloveppt-builder Step 4 / audience 填补了"内容自检 / 视觉资产 / 读者认知"三个独立的质量门。详见 [agent-internals.zh.md](agent-internals.zh.md)。
...
iLovePPT 只有**一个**标准入口:**直接对话主线程 Claude**。主线程识别 PPT 意图后**自动建 team + 派发 brainstorm agent**,后面 5 agent + 1 旁路接力,你不用记 agent 名字。
...
字段收齐后 brainstorm 写 `brief.md` 等你 OK,然后主线程接力派 `iloveppt-author` 出 outline → `iloveppt-critic` 评 → 审 outline → author 拓 content → critic 评 → 审 content → `iloveppt-builder`(build + Step 4 视觉一气呵成)→ `iloveppt-audience` 评分,直到 ≥ 9 分。
...
| 已有审过的 brief.md / content.md,想跳到下一步 | 把 brief.md / content.md 路径告诉主线程,主线程根据状态派 author / iloveppt-builder |
| 已有审过的 content.md,只想跑 build + 视觉 + audience | 直接说"按 content.md build 一份",主线程派 iloveppt-builder mode=full |

**直接 `@agent-iloveppt-builder` 但缺 content.md 会被 reject** —— iloveppt-builder 不做 brief 解析、不写 content。
...
├── builder/                             ← iloveppt-builder 全部产物(机械 build + Step 4 视觉)
│   ├── deck_plan.json                    (机械接缝,Step 3 自动调整字数/排版都改这里;可手改重 build)
│   ├── deck_v1.pptx                      (最终产物)
│   ├── deck_v1_render/                   (渲染图,QA 用,可删)
│   ├── visual_report_r1.md               (iloveppt-builder Step 4 视觉优化报告)
...
└── _assets/                             ← 你提供的素材,跨 agent 共享
    ...
    ├── brand/                             (品牌 assets,可选,iloveppt-builder Step 4 优先用)
    └── refs/                              (参考图)
```
...
你: "批准 outline,继续"
   ↓
[critic Stage C · 14 项 + 5 维度判断性评审 outline]
   ↓
...
你: "批准 content,继续"
   ↓
[critic Stage D · 14 项 + 5 维度判断性评审 content]
   ↓
   ├── pass / pass_with_notes → 派 iloveppt-builder
   └── needs_revision → 你 cherry-pick → 派 author 改 content
   ↓
[Stage E · iloveppt-builder · Step 0-3 构建 .pptx + 17 项机械视觉 QA × ≤ 3 轮 + Step 4 主动加视觉(iconify / Unsplash / brand / RAG patterns)]
   ↓
[Stage F · audience 4 维度评分(9 分硬阈值)]
   ↓
   ├── ≥ 9 分 → 主线程展示交付清单(checkpoint 4)→ 你 OK 交付(checkpoint 5)
   └── < 9 分 → 反馈三类分流(author rewrite / builder mode=visual_redo / theme_fix)→ 修复环 ≤ 5 轮
```
...
> 批准 outline 后进 critic Stage C 评审(Pyramid 单点收口在 critic Section A 7 项),critic 可能反馈"论据强度弱 / 节奏感差 / 措辞模糊 / 平衡问题 / pattern 不匹配",你可 cherry-pick 修。
...
content 批准 + critic Stage D pass 后,主线程派 **iloveppt-builder**(build + Step 4 视觉一气呵成)→ **audience**。整段跑完返回:

```yaml
# iloveppt-builder 返回
pptx_path: /abs/path/to/deck_v1.pptx
qa_rounds: 2
deck_plan_edits:                       # Step 3 自动改了 deck_plan.json 的清单(content.md 全程不动)
  - page: 5
...
review_needed_pages:                   # 3 轮仍未解决的(罕见)· category 标 architectural / needs_author_rewrite
  - page: 12
    category: architectural
    issue: "matplotlib 字体 fallback"
    suggestion: "macOS 装雅黑"
visual_edits: [...]                    # Step 4 主动加视觉(留下的 icon / hero / brand assets)
rolled_back: []                        # Step 4 加了反而糟的(自动回滚)

# audience 返回
overall_score: 9.2
ready_for_delivery: true               # ≥ 9 且无 needs_major
top_3_must_fix: [...]                  # 可选改进建议(不阻塞交付)
```

**主线程会问**:"iloveppt-builder 在 Step 3 自动调整了 1 处(page 5 action title 缩短)、Step 4 加了 8 处 icon、audience 给 9.2 分。要看 audience top 3 建议吗?接受交付 / 还想再改 / 回退某条 deck_plan edit?"
...
**iLovePPT 的内容设计核心要求**:整份 deck 按麦肯锡金字塔原理组织。critic Stage C Section A 跑 7 项 Pyramid 审计 —— 不通过 author 就要 rework outline。你 brief 里把这五件套讲清楚,critic Stage C 几乎一次过。
...
**提案 / 路演 / 汇报 / 评审场合一律走金字塔** —— author / critic / iloveppt-builder 三层 Pyramid 防线不接受"先放着"含糊过关。data_report / tutorial / catalog 等纯展示场景可在 brief 里说明,author 自检会软阻塞(可豁免附理由)。
...
写 brief 时 outline 直接用 action title 句式,Stage C 出 outline 时几乎不用改。**这同时也满足金字塔的纵向疑问/回答链**——把所有 action title 抽出来按顺序读,应该能讲出顶端论点的完整论据链(critic Section A 第 6 项 · ghost deck test)。
...
### 7.1 论证骨架(critic Section A 7 项 Pyramid 审计)

批准 outline 后 critic Stage C 跑 Pyramid 7 项审计(单点收口)+ 5 维度判断性评审。你预审 outline 时把这 7 条过一遍能让 critic 一次过:
...
```
A4 MECE fail:第 3 节"评审流程"和第 4 节"组织保障"内容有重叠
...
2. **`deck_plan_edits[]`** —— iloveppt-builder 在视觉 QA 循环里自动改了哪些 deck_plan.json 字段(content.md 全程不动)
3. **`review_needed_pages[]`** —— 3 轮自检仍解不掉的问题(罕见)

### 处理 deck_plan_edits

iloveppt-builder 修了 deck_plan.json 的字数 / 排版问题(content.md SSOT 不可变),都会列在这里:
...
| 选项 | 怎么做 |
|---|---|
| 全部接受 | "接受所有 deck_plan_edits"(.pptx 用调整后的版本) |
| 回退某条 | "回退 page 5 的改动"(主线程会 revert 那一条 deck_plan 字段) |
| 升 author | 若你觉得 iloveppt-builder 的截短偏移语义 → "page 5 让 author 重写"(走 needs_author_rewrite 路径) |

### 处理 review_needed_pages

### 8.1 review_needed_pages 清单

iloveppt-builder 跑完 Stage E 给你的清单大概这样:
...
```
@agent-iloveppt-builder 第 7 页 cards 改成 4 个,加一张"客户案例"卡片,
重跑 Stage E iloveppt-builder(outline 已批准,不用回 Stage C)
```

agent 会跳过 Stage A-D,从 Stage E iloveppt-builder 开始重跑。
...
| 角色 | 色值 | 用在哪 |
|---|---|---|
| `BRAND_PRIMARY` | `#0A52BF` | 标题、强调装饰、key icon(AAA 7:1) |
| `BRAND_DARK` | `#0B2A4A` | 封面背景、深色文字 |
| `BRAND_TINT` | `#E6F0FC` | 卡片底、tag 背景 |
| `ACCENT` | `#007A6D` | 极个别强调点(AA 5.2:1) |
...
装好后让 agent 重跑 Stage E iloveppt-builder,或自己跑 `build.py`。
...
review_needed_pages 一定会标这条,按 suggestion 改即可。
...
```
@agent-iloveppt-builder 这份 outline 全是文字,按 diagram-planning 的 4 类图决策表
...
### 11.6 review_needed_pages 三轮还修不好

**症状**:agent 跑了 3 轮自检,某页仍标 `review_needed_pages` 含 `category: architectural`。
...
```
@agent-iloveppt-builder 第 5 页用 single_focus 装太多内容了,
改成 bullet_list 重跑
```
...
```
@agent-iloveppt-builder 这次只要 .pptx,跳过视觉自检
```
...
| 术语 | 意思 |
|---|---|
| **主线程 Claude** | 与你直接对话的 Claude;**thin dispatcher** —— Phase A 用 `TeamCreate(brainstorm)` 建 team 让 brainstorm 跟你聊;Phase B brief 批准后用 `Task` 依次调 author/critic/iloveppt-builder/audience subagent,不持有 PPT 业务逻辑 |
...
| **iloveppt-critic**(第 3 agent) | Stage C/D 双 gate —— outline 评审 + content 评审。14 项 checklist + 5 维度判断性评审。verdict ∈ {pass, pass_with_notes, needs_revision} |
| **iloveppt-builder**(第 4 agent) | Stage E —— 接 content.md → critic gate 验过 → md→JSON → build → 17 项机械视觉 QA → Step 4 主动加视觉(iconify / Unsplash / brand / RAG patterns 四路)。单次派发完成 build + 视觉 |
| **iloveppt-audience**(第 5 agent) | Stage F —— 模拟目标受众读 deck 评分,9 分硬阈值。反馈分 3 类:needs_author_rewrite / needs_visual_redo(派 iloveppt-builder mode=visual_redo)/ needs_theme_fix |
...
| **outline.md** | Stage C 产出物(`deck_v{N}_outline.md`),章节 + per-chapter pattern_hints,等你批准;Pyramid 由 critic Section A 判定 |
...
| **deck_plan_edits** | iloveppt-builder 在视觉 QA 循环里自动改了 deck_plan.json 的清单(author/content.md 全程不动),返回时给你看,可批准/回退 |
| **_assets/** | 素材文件夹,raw(原始)/ charts(生成图)/ refs(参考图)/ icons(iloveppt-builder Step 4)/ hero(iloveppt-builder Step 4)/ brand(用户自带)等子目录 |
...
| **Pyramid 7 项审计** | critic Section A 的 7 项金字塔结构审计(单点判定);任一不过 → critic verdict 触发 needs_revision,用户 cherry-pick 让 author 改 |
| **ghost deck test** | Pyramid 第 6 项:把所有 action title 抽出来按顺序读,能不能讲出顶端论点的完整论据链 |
...
| **review_needed_pages** | Stage E iloveppt-builder 输出的"需要人审"清单,3 轮自检改不动的页;category 标 `architectural`(layout/内容架构性问题)或 `needs_author_rewrite`(文字层需 author 重写)|
| **diagram_plan** | Stage C 产出的图层规划,标出哪几节配什么类型的图、用什么工具 |
| **template-extract** | 用户给 .pptx 模板时,iloveppt-template-extractor 走完整 ingest:复制 _source → render_pages.py 渲染每页 → 起草双层 meta.yaml(draft)→ 用户审 → 主线程 embed 入 DB |
...
| 想了解 | 看 |
|---|---|
| agent 的完整设计与约束 | `${CLAUDE_PROJECT_DIR}/.claude/agents/iloveppt-builder.md` |
...
| 13 layout 文案规则 + **金字塔原理 5 件套** + Pyramid 7 项审计表(critic Section A 跑)| `${CLAUDE_PROJECT_DIR}/.claude/skills/pptx-deck/content-writing.md` |
| 图层规划 4 类决策表 | `${CLAUDE_PROJECT_DIR}/.claude/skills/pptx-deck/diagram-planning.md` |
| 视觉自检 17 项 checklist | `${CLAUDE_PROJECT_DIR}/.claude/skills/pptx-deck/visual-qa.md` |
...
*适用流水线:5 agent + 1 旁路(brainstorm / author / critic / iloveppt-builder / audience · template-extractor)*
