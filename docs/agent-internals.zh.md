...
> 这份文档讲清楚 iLovePPT **怎么工作的** —— 架构 + 能力库存 + 6 个主 agent + 2 个 Haiku helper 细节 + 关键设计决策。适合想理解或改造系统的人;不是用户操作手册(那个看 [`${CLAUDE_PROJECT_DIR}/docs/MANUAL.zh.md`](${CLAUDE_PROJECT_DIR}/docs/MANUAL.zh.md))。
>
> **当前版本 v0.13.0(P0+P1+P2+P3 phase 1/2/3 全完成)** · 全维度优化基准见 [`docs/optimization/2026-05-27-iLovePPT-optimization-plan.md`](${CLAUDE_PROJECT_DIR}/docs/optimization/2026-05-27-iLovePPT-optimization-plan.md)
>
...
**v0.13.0 后**:主流水线 6 agent(全 opus)+ 1 旁路 extractor(opus)+ 2 新 Haiku helper(self-check / yaml-fixer · P3-7 路由 · 错误恢复路径专用,主流水线不直接派)。

| agent | 一行简介 | 调用方式 | model |
|---|---|---|---|
| `iloveppt-brainstorm` | Stage A-B:多轮对话收 brief + 素材 + **Step 3.6 brief self-audit**(原 critic Stage B 5 项已并入 · P2-3.1)+ skeleton 检测(P3-8)+ inspirations 持久化(P2-8 · sha256)+ cost_budget 收集(P3-17)+ 多模板组合 deck 4 schema(P3-9) | **TeamCreate**(team,持续窗口) | opus |
| `iloveppt-author` | Stage C 出 outline.md + 自走 Stage D 拓 content.md(P2-3.2 后**无中间 critic gate**)+ resolve_theme 3 schema 解析(P3-9)+ chapter-aware preferred-template | **Task**(subagent) | opus |
| `iloveppt-critic` | **stage=cd 合审单次**(C+D merged · P2-3.2)+ 21 项量化(A 7 + B 9 + J 5 · severity 0-3 + evidence ≥ 10 字 + suggestion · P2-1)+ verdict 自动算公式 + critic-rubric.yaml SSOT + Step 0.5 hot-reload chapter_hashes carry over(P2-4) | **Task**(subagent) | opus |
| `iloveppt-builder` | Stage E:Step 0.3 hot-reload(P2-4)+ Step 0.5 SSOT verify(content.md vs deck_plan.json sha256 · P2-5)+ Step 1.0 多模板 brief.theme 解析(P3-9)+ 跨模板 tier1 deep-copy 保 primary theme 字体 + Step 4.3.5 视觉一致性反查(image RAG · P2-7)+ Step 4.5 query_cache(P2-10)+ 4 路降级(brand / iconify / Unsplash / RAG patterns) | **Task**(subagent) | opus |
| `iloveppt-audience` | **Step 0.0 spot-check**(原主线程 spot-check 已并入 · P2-3.3)+ Step 0.4 hot-reload(P2-4)+ **每页 12 项 × 0-3 量化**(P2-2)+ multi-persona strict-eval(取最低分 · P2-13)+ Step 3.5.2 image RAG 反查 alternative(P2-7)+ **Step 3.6 feedback writeback**(opt-in · P3-5)| **Task**(subagent) | opus |
| `iloveppt-template-extractor` | 旁路 Stage T:复制 _source + render_pages.py 渲染每页 + Step 2.5 declared/rendered 对账 advisory + Step 2.6 watermark detect(P3-22)+ Step 3.1 LLM 起草双层 meta.yaml.draft(17 layout enum + other 兜底) + Step 3.3 **14 项 self_check**(P3-15 #14 SOURCE_PPTX_SHA_DRIFT)→ user_review_drafts gate → 主线程 embed | **Task**(subagent) | opus |
| `iloveppt-self-check` **(NEW · P3-7)** | 错误恢复 helper:跑 `red_line_check.py` / `extractor_self_check.py` 解析 stdout,归一化 yaml 报告。**主流水线不直接派**,只在主线程错误恢复路径上 dispatch | **Task**(subagent) | **haiku-4-5** |
| `iloveppt-yaml-fixer` **(NEW · P3-7)** | 错误恢复 helper:修 LLM(其他 agent)写错的 YAML(quote / indent / enum 字面)。**只调字面不动语义**,主流水线不直接派 | **Task**(subagent) | **haiku-4-5** |

**模型分层 rationale**:
- 主流水线 6 agent 全 opus —— 团队复杂度高,横跨多轮对话 / 深度判断 / 视觉认知 / 模板摄入,任一环掉链传下游;cost 较分层方案高 ~3-4x 可接受
- 2 Haiku helper —— P3-7 路由优化:跑校验脚本 + 修 yaml 字面是窄任务(无创造性推理),用 Opus 单价 19x Haiku 浪费;主线程错误恢复路径专用,**严禁 subagent 起 subagent**(详 §4 决策 9)
...
*含 cherry-pick gate + 三类反馈分流 · P2-3 后 9→7 步精简*
...
*hosted multimodal RAG · 跨 5 agent 共享检索入口 · P2-6 完整化*

**用途**:Visual Patterns 库是输出**高质量 PPT** 的关键知识库 —— **15 个**跨模板视觉模式(P2-6 ingest:timeline-h-5 / timeline-v-4 / pdca-4-cycle / funnel-4-tier / cards-flag-4 / comparison-tier-3 / venn-3-circle / radial-5-spoke / pyramid-3-layer / pyramid-maslow-5 / process-arrow-5 / 2x2-priority-matrix / swot-quadrant / bullet-5-numbered / single-focus-kpi),每个 pattern 含 `meta.yaml`(元数据 + fallback_rendering)+ `preview.png`(预览图)。

**底层实现**:阿里云 tongyi-embedding-vision-plus(dim 1152,text + image 同 API)+ sqlite **WAL mode + busy_timeout 10s**(P3-6 · 并发不锁)。`library/_rag/db.sqlite` gitignored。**唯一检索入口** `library/search.sh` 跨双 kb 统一 router。

**4 mode 检索**(P2-7 image RAG + P2-9 ablation-driven hybrid + P3-5 feedback loop):

```bash
# Mode 1 · text:跨双 kb 默认搜
bash ${CLAUDE_PROJECT_DIR}/library/search.sh \
     --query "<intent 关键词>" --top-k 5 --format json

# Mode 2 · hybrid:text + 关键字 bm25 权重 (P2-9 默认 0.8/0.2 · ablation-driven)
bash ${CLAUDE_PROJECT_DIR}/library/search.sh \
     --query "..." --hybrid-weights 0.8,0.2

# Mode 3 · image RAG(P2-7):图片相似度查模板/页
bash ${CLAUDE_PROJECT_DIR}/library/search.sh \
     --query-image "<abs-path-to-inspiration.png>" --top-k 5

# Mode 4 · preferred-template(模板优先 fallback vp,brainstorm 选定模板 theme 时用)
bash ${CLAUDE_PROJECT_DIR}/library/search.sh \
     --query "..." --preferred-template <theme> --type page --top-k 5

# 可选 · feedback-aware(P3-5 · opt-in 软降权)
bash ${CLAUDE_PROJECT_DIR}/library/search.sh \
     --query "..." --feedback   # avg<7.0 + count>=3 的 pattern × 0.9 软降权
```
...
| image RAG | ✓ Step 1 inspiration 反查模板 (P2-8) | — | — | ✓ Step 4.3.5 视觉一致性反查 (P2-7) | ✓ Step 3.5.2 needs_visual_redo 页反查 alternative (P2-7) |
| preferred-template | ✓ Stage A 模板候选 | ✓ Stage D 拓写 | — | — | — |
| feedback-aware | ✗ 默认关 | ✗ 默认关 | ✗ 默认关 | 可选(用户开)| — |

**性能优化**:
- **embedding batch API**(P1-6/7):text=8 / image=4 batch · 7 模板 ingest 时间 ~5min → ~215s(parallel)
- **WAL mode**(P3-6):并发读写不锁,busy_timeout 10s
- **query cache**(P2-10):iconify / Unsplash query 沉淀到 `library/_rag/external_query_cache.jsonl`,fuzzy match 复用
...
- **196 tpl_pages 入 DB**(已清 16 张工具页污染 · P0-6)
- **placeholder_map 含 shape_id**(P0-7):212/212 全 backfilled
...
- `items/<name>/meta.yaml`(模板级 metadata · 入 git · 含 `provenance.source_pptx_sha256` 用于 P3-15 drift 检测)
...
- L4 视觉分析 + 起草 + self-check(extractor LLM 必跑):
  ...
  - **Step 2.6**:**watermark + 第三方品牌 LOGO detect**(P3-22 · `library/_rag/scripts/detect_watermark.py`):扫描 _source/*.pptx + 渲染 PNG · 命中已知品牌词 / 半透明大图层 → 强警告 + needs_manual_review 标记
  - **Step 3.3 self-check 14 项**(进 Step 4 前 hard gate · `library/pptx-templates/scripts/extractor_self_check.py`):
    1. REQUIRED_TEMPLATE_FIELDS · 模板级必填字段
    2. REQUIRED_PAGE_FIELDS · 页级必填字段
    ...
    7. EXTRACTION_MATH_INCONSISTENT(declared/rendered/discrepancy)
    8. TEMPLATE_NAME_MISMATCH
    9. PMAP_TREE_PATH_UNRESOLVABLE
    10. LIST_ELEMENT_TYPE_FAILED · list 字段每元素必须 str(P0-1)
    11. SHAPE_ID_RESOLVE_FAILED · placeholder_map slots[].shape_id 必须在 source pptx 找到(P0-7)
    12. VARIANT_ENUM_FAILED · variant ∈ `layout_variants.yaml`(P1-1)
    13. SLOT_ID_ENUM_FAILED · slots[].id ∈ `slot_ids.yaml`(P1-2)
    14. **SOURCE_PPTX_SHA_DRIFT** · _source/*.pptx sha256 == meta.provenance.source_pptx_sha256(P3-15)
   任一项不通过 → `status: error` + `code: <项 code>` hard_stop。错误恢复路径可派 `iloveppt-self-check` Haiku helper 跑并解析 + `iloveppt-yaml-fixer` Haiku helper 修字面(P3-7)
...
| `categories.yaml` | 12 enum(4→12 retrofit · P1-3)· enterprise-product / enterprise-strategy / creative-aurora / finance-arrow 等 | extractor / brainstorm RAG category filter | embed.py 强制 |
...
| `keywords_bank.yaml` | 13 桶 / 327 关键词 · 按 category 分桶 · LLM 从桶里选不许自由发明 · `expansion_hints.yaml` 是其 derived view(P1-5/9)| extractor / author keyword | yaml-loaded EXPANSION_HINTS |
...
**chapter_hashes 用途**(P2-4):author rework 单章后,critic / builder / audience 可读 state.json 对比当前 content.md 各章 hash 跟上次:**没变的章节** carry over 上次 verdict / 评分(从 prev report 或 chapter_*_carryovers 里读);**hash 变了** 才重做。极大减少 rework 时间(~30min → ~5min)。

**向后兼容**:旧 deck 的 state.json 没 chapter_hashes 字段也 OK(fallback 全重算);新 deck 派发前主线程跑 `compute_chapter_hashes.py` 写字段。

**cost block(P1-8 + P3-17)· 任何 state.json 都可挂**:
...
- `model` — 当前价格档(默认 `opus`;**P3-7 Haiku 路由后** self_check / yaml_fixer 按 haiku-4-5 单独计价)
...
| author | `dispatch_critic` | Task(critic, args 含 **stage=cd**(P2-3.2 合并) + outline_md_path + content_md_path)|
...
| iloveppt-builder | `hard_stop` | 展示 errors 给用户三选一(按 suggestion 改 / 终止 / 自己指令);可派 `iloveppt-self-check` Haiku helper 跑校验脚本 + `iloveppt-yaml-fixer` 修字面(P3-7) |
...
| audience | `needs_theme_fix` | 主线程改 themes/*.yaml(P3-1 yaml 化后)|
...
*P0-2 / P0-3 / P2-11/12 / P3-5 / P3-21 数据基础层*

**用途**:RAG 决策、agent token 成本、layout failure rate、deck 间趋势全部可观测,优化不再瞎拍。

| 工具 | 用途 | 维护方 | 关键路径 |
|---|---|---|---|
| `library/_rag/query_log.jsonl` | 所有 RAG query 留底(query / hits / scores / chosen / ts)· **P3-21 redact 默认开**(邮箱 / 手机号 / 金额 redact) | search.sh 自动写 | gitignored |
...
| `library/_rag/feedback.jsonl` | audience 评分后 append `(chosen_pattern_id, audience_score, page, deck)`(P3-5 opt-in)| audience Step 3.6 append | gitignored;**全仓库共享**,不在 deck 工作目录 |
| `scripts/dashboard.py` | 跨 deck 聚合:token / rework / audience / layout failure rate(P2-11/12) | 手动跑 | stdout report |
| `library/_rag/scripts/track_cost.py` | per-deck cost log + budget warn(P1-8 + P3-17) | agent return 后 hook 调 | state.json `cost` block |
| `library/_rag/scripts/feedback_stats.py` | feedback.jsonl 聚合(哪些 pattern 低分 + count) | dashboard 调 | stdout |
| `library/_rag/scripts/redact.py` | query / brief 脱敏 helper(P3-21) | query log + brief 写盘前调 | inline |
| `library/_rag/scripts/rotate_api_key.py` | API key rotation 工具(P3-18) | 手动跑 | `.env` rewrite |
| `library/_rag/scripts/ablation_hybrid_weights.py` | hybrid 权重 ablation(P2-9 用于定 0.8/0.2 default) | sprint 末跑 | bench_results |

**P3-5 feedback loop opt-in**:
...
- skip query log → 后续 RAG 改动全瞎拍(P0-2/3 强制前置)
...
主流水线 6 agent(brainstorm / author / critic / builder / audience / extractor)在 §3.1-3.6;新增 2 个 Haiku helper(self-check / yaml-fixer · P3-7)在 §3.7。
...
| **读哪些 markdown** | brief.md(已写后续轮读)/ user-provided 素材文件 / `library/pptx-templates/items/<name>/meta.yaml`(若用户选用已 ingest 模板)/ `library/deck-skeletons/<id>/` skeleton.yaml(P3-8 · scripts/new_deck.py 起新 deck 时写入 brainstorm/skeleton_used.yaml)/ `library/vocabularies/audience_personas.yaml`(P1-4 · audience list 校验)|
| **写哪些 markdown** | `brainstorm/brief.md`(唯一写者)/ `brainstorm/state.json` / `brainstorm/inspirations/<sha256-short>.png`(P2-8 · 用户 paste 图持久化)|
| **调 RAG**(`library/search.sh`) | ✓ Stage A 列模板:`--kb pptx-templates --type template --query <主题>`;✓ Step 3.5 跨 kb 取 top-5 category;✓ Step 1 用户给 inspiration 图 → `--query-image <path>` 反查模板(P2-8 image RAG) |
...
跟用户多轮对话挖需求 + 收素材清单 → 写 brief.md → **跑 Step 3.6 brief self-audit(5 项 · P2-3.1 原 critic Stage B 已并入)** → 等用户确认 → 派 author。

**必收齐 6 字段**:`audience`(list[persona] · P2-13)/ `duration_min` / `top_recommendation` / `theme`(str/list/dict · P3-9 4 schema)/ `output` / `presentation_mode` / `cost_budget_usd`(P3-17 · 默认 10)。
...
**多模板组合 deck(P3-9 · 4 schema)**:
- (a) 单模板(默认):`theme: tech_blue` / `theme: <短名>`
- (b) inspiration 图 → image RAG 反查模板(用户给图非 .pptx)
- (c) **多模板组合**:进入子分支,3 schema:
  - schema 1 · 全 list:`theme: [tech_blue, finance_arrow]` 按 chapter 顺序循环用
  - schema 2 · default + overrides:`theme: {default: tech_blue, overrides: {3: finance_arrow}}`
  - schema 3 · 全 mapping:`theme: {1: tech_blue, 2: finance_arrow, 3: ...}`
- **不准瞎猜**:用户给 list 但没说"哪章用哪个" → **必须追问**

**skeleton 检测(P3-8)**:
- 检测 `<working_dir>/brainstorm/skeleton_used.yaml`(`scripts/new_deck.py --skeleton <id>` 起新 deck 时写入)
- 存在 → Read + parse → 写 `state.skeleton`(原文存档),用 `suggested_*` 默认值填 collected(只对 collected 缺失字段填,**不覆盖**用户已答)
- 6 个内置 skeleton:`annual_strategy_review / customer_pitch / product_launch / project_postmortem / quarterly_finance_report / team_okr_kickoff`

**inspiration 图持久化(P2-8)**:
- 用户 chat paste image → cp 到 `<working_dir>/brainstorm/inspirations/<sha256-short>.<ext>`(sha256 短 hash 防重复)
- `state.inspirations` append 记 path + pasted_at + used_in_image_rag
- 后续 image RAG 反查模板 / page 优先用此持久化路径

**cost_budget 收集(P3-17)**:
- brief 收集时问 `cost_budget_usd`(默认 10);写到 brief.md frontmatter + state.collected
- builder/audience 跑时 track_cost.py 50/80/100% warn,超额暂停 + 问用户继续 / 终止
...
status: ok
next_action: dispatch_author          # P2-3.1 后:不再走 dispatch_critic_brief
artifacts:
  - {path: <abs to brief.md>, kind: brief_md}
brief_summary: <一句话>
pattern_hints_for_author: [process, cycle, comparison]   # Step 3.5 RAG 预选
brief_audit:                          # P2-3.1 Step 3.6 self-audit 结果
  verdict: pass | pass_with_notes
  items:
    - {id: B1, name: SCQA, passed: true, severity: 0, evidence: "..."}
    - {id: B2, name: audience_persona_list, passed: true, severity: 0}
    - {id: B3, name: theme_schema, passed: true, severity: 0}
    - {id: B4, name: red_line_words_fuzzy, passed: true, severity: 0}
    - {id: B5, name: cost_budget, passed: true, severity: 0}
skeleton_used:                        # P3-8 · null 或 skeleton.id 记 audit trail
  id: quarterly_finance_report
  defaults_overridden: [audience, theme]    # 用户改了哪些 suggested_*
...
| 维度 | iloveppt-author |
|---|---|
| **调用方式** | Task(subagent · Phase B · Stage C → Stage D 一次性走完无中间 critic gate · P2-3.2)/ rework 独立 Task |
| **模型** | opus |
...
| **读哪些 markdown** | brief.md / outline.md(Stage D)/ content.md(rework)/ content-writing.md / diagram skill docs / `library/pptx-templates/items/<theme>/meta.yaml`(若 ≠ tech_blue · 含 multi-template 时多个)/ `library/visual-patterns/items/<id>/meta.yaml`(search.sh 返回 meta_path 后 Read)/ `library/vocabularies/keywords_bank.yaml`(P1-5 keyword 桶)|
...
| **跨 agent handoff 出口** | `ask_user_for_outline_approval`(Stage C 完)/ `ask_user_for_content_approval`(Stage D 完)/ `dispatch_critic`(用户审批 content.md 后 · **stage=cd** 合并合审 · P2-3.2)|

#### 职责

基于 brief + 素材清单,按金字塔原理 出 outline.md(Stage C)+ 拓写 content.md(Stage D)。**P2-3.2 后 Stage C → D 一次走完,中间无 critic gate**;rework 走独立 Task。
...
**P3-9 multi-template resolve_theme**(Stage C/D 拿到 chapter_index 后查对应 theme):
...
| 维度 | iloveppt-critic |
|---|---|
| **调用方式** | Task(subagent · Phase B · **stage=cd 合审单次** · P2-3.2 后)|
...
| **state file** | **无**(每次派发独立,产出全在 critic_report_*.md · 读 author/state.json 取 chapter_hashes hot-reload · P2-4)|
| **读哪些 markdown** | brief.md / outline.md / content.md / content-writing.md / `library/visual-patterns/items/<id>/meta.yaml` 或 `library/pptx-templates/items/<name>/pages/<NN-slug>/meta.yaml`(J5 反查)/ **`.claude/agents/critic-rubric.yaml`**(P2-1 量化 SSOT)/ author/state.json + prev critic_cd report(Step 0.5 hot-reload)|
...
**partner 评审员而非合规检查员**。**P2-3.2 后 stage=cd 合审一次**(原 Stage C + Stage D 合并),C+D 合并为单次评审看 outline + content + 5 维度判断。**P2-1 每项强制量化 schema**,verdict 由公式自动算,LLM 不主观判。
...
stage: cd                                                    # P2-3.2:唯一支持值
verdict: <same as next_action · 公式自动算>
artifacts:
  - {path: <abs to deck_v{N}_critic_cd.r{R}.md>, kind: critic_report}
items_21:                                                   # P2-1 量化 schema
  - {id: A1, name: 单一顶端论点, passed: true, evidence: "...", severity: 0, suggestion: ""}
  - {id: A2, name: SCQA, passed: true, evidence: "...", severity: 0, suggestion: ""}
  # ... 21 项总数(A1-A7 + B1-B9 + J1-J5)
verdict_calculation:                                        # 公式算依据(给主线程审计)
  block_count: 0          # severity==3 数
  warn_count: 1           # severity==2 数
  nit_count: 2            # severity==1 数
  threshold_hit: notes    # block / warn_accumulation / notes / pass
rounds_used: <int>
suggested_alternative_patterns:                             # advisory(J5 输出)
  - {page: N, current: <id>, suggest: <id>, reason: "..."}
hot_reload:                                                 # P2-4 Step 0.5 用了则填
  used: true | false
  carried_over_chapters: [1, 2]                             # carry over 哪些章
  rerun_chapters: [3]                                       # 重跑哪些章
```
...
| **state file** | **无**(单次派发跑完,状态全在 visual_report_r{N}.md + deck_plan_edits;读 author/state.json Step 0.3 hot-reload · P2-4)|
| **读哪些 markdown** | deck_v{N}_critic_cd.r{R}.md(Step 0 必读 gate · P2-3.2 后)/ content.md / content-writing.md / visual-qa.md / `library/pptx-templates/items/<name>/meta.yaml`(若 theme ≠ tech_blue · multi-template 时每个 chapter 各自 resolved theme)/ `library/{visual-patterns,pptx-templates}/items/<id>/meta.yaml`(读 `<!-- pattern: vp:/tpl: -->` 注释时按前缀查对应 kb)/ `themes/<name>.yaml`(P3-1 yaml 化后 · _schema.yaml 验证)|
| **写哪些 markdown** | `builder/deck_v{N}.pptx`(通过 build.py · 现拆为 `builder/{base,tier1,tier2,tier3}.py` · P3-3)/ `builder/deck_v{N}_plan.json`(Step 3 的字数/视觉修复全落在这里 · 含 `derived_from_sha256` · P2-5)/ `builder/deck_v{N}_visual_qa.r{R}.md` / `builder/rag/<page_id>.source.yaml`(每张 RAG 嵌入的 source 追溯 · 图片资产 reproducibility)|
| **调 RAG** | ✓ Step 2 解析 `<!-- pattern: vp:/tpl: -->` 注释 → 查对应 kb DB;✓ **Step 4.3.5 image RAG 视觉一致性反查**(P2-7 · rendered PNG → top-5 模板页相似度);✓ Step 4.2.5 第 4 路 fallback;✓ **Step 4.5 query_cache**(P2-10 · iconify/Unsplash fuzzy match 复用) |
| **调 templates 库** | ✓ Step 1.0 P3-9 多模板 brief.theme 解析 → builder/base.py `parse_theme()` → `ThemeSpec`(per-chapter resolved theme + cross-pptx tier1 deep-copy);✓ build.py 解析 theme 字段 + 跨模板 tier1 复用保 primary theme 字体/色板 |
| **advisory 来源** | 无 |
| **是否唯一写者** | **deck_plan.json / .pptx / visual_report 唯一写者**;**author/content.md 全程不可变**(无 postbuild 副本)|
| **跨 agent handoff 出口** | `dispatch_audience`(成功)/ `hard_stop`(critic_d_missing / critic_d_not_passed / **ssot_verify_failed** · P2-5 / missing_layout_directive / qa_3_rounds_exhausted)|

#### 职责

**Stage E:5 步一气呵成**:Step 0 critic gate → Step 0.3 hot-reload(P2-4)→ **Step 0.5 SSOT verify**(P2-5)→ Step 1 strict md→JSON + **Step 1.0 P3-9 多模板 brief.theme 解析** → Step 2 build.py 出 .pptx + render PNG → Step 3 17 项机械视觉 QA(≤ 3 轮)→ Step 4 主动加视觉(4 路降级 + **Step 4.3.5 image RAG 反查** + **Step 4.5 query_cache**)。

**Step 0 critic gate**:必须先 Read critic_cd_report_path(主线程传 _r{R} 路径)→ verdict ∈ {pass, pass_with_notes} 才进 Step 1;needs_revision 或 missing → `hard_stop`。**iloveppt-builder 不跑 Pyramid 自检**,信任 critic stage=cd 那道 gate(P2-3.2 后)。
...
**Step 1.0 P3-9 多模板 brief.theme 解析**(per-chapter 路由):
...
**Step 4.3.5 视觉一致性反查(P2-7 · image RAG)**:
...
ssot_verify:                          # P2-5 Step 0.5
  enabled: true | false
  derived_from_sha256_match: true | false
hot_reload:                           # P2-4 Step 0.3
  used: true | false
  carried_over_chapters: [1, 2]
  rerun_chapters: [3]
theme_spec:                           # P3-9 Step 1.0
  mode: single | list | dict_default | dict_mapping
  resolved_per_chapter: {1: tech_blue, 2: finance_arrow}
visual_step4:
  capability:
  ...
  visual_consistency_check:           # P2-7 Step 4.3.5
    enabled: true | false
    preferred_template: <brief.theme>
    visual_drift_pages:               # top-1 score < 0.6
      - {page: N, current_pattern: ..., closest_template_page: ..., score: 0.55}
  query_cache:                        # P2-10 Step 4.5
    cache_path: library/_rag/external_query_cache.jsonl
    hits: 3
    new_writes: 1
  visual_edits:                       # 图片资产 reproducibility(强制 source 配对)
    - {asset: builder/rag/p5.png, source: builder/rag/p5.source.yaml, tool: rag}
    - {asset: builder/icons/check.png, source: builder/icons/check.source.yaml, tool: iconify}

# 失败:
agent: iloveppt-builder
status: error
next_action: hard_stop
errors: [{code: critic_d_missing | critic_d_not_passed | ssot_verify_failed | missing_content_md | missing_layout_directive | qa_3_rounds_exhausted, message, suggestion}]
```
...
|---|---|
| **调用方式** | Task(subagent · Phase B · 每轮新派发,无状态;支持 multi-persona list[str] · P2-13)|
| **模型** | opus |
| **Tools** | Read / Glob / Write / Bash(**无 Edit / WebSearch** · 评分阶段只读 PNG · Bash 仅用于 spot-check + image RAG + feedback writeback)|
| **state file** | **无**(每轮独立,产出在 audience_review_r{N}.md;读 author/state.json Step 0.4 hot-reload · P2-4)|
| **读哪些 markdown** | content-writing.md / visual-qa.md / library/visual-patterns/INDEX.md / **render PNG 全 N 页**(唯一接触 .pptx 产物的方式)/ `library/vocabularies/audience_personas.yaml`(P1-4 persona SSOT)/ `<working_dir>/builder/deck_v{N}_plan.json`(**仅 Step 0.0 spot-check + Step 3.6 feedback writeback,不影响评分**)/ `<working_dir>/brainstorm/brief.md`(Step 0.5 red_line 双语 fuzzy + 拼音检查 · P1-10)|
| **写哪些 markdown** | `audience/audience_review_r{N}.md`(唯一写者)/ `library/_rag/feedback.jsonl`(P3-5 · append-only · **全仓库共享 不写 deck 工作目录**)|
| **调 RAG** | ✓ Step 3.5 文本反查(text RAG top-3 选 alternative);✓ **Step 3.5.2 image RAG 反查**(P2-7 · rendered jpg → top-5 模板页相似度 → 找视觉一致性更高的 alternative)|
| **调 templates 库** | **不用** |
...
**P2-2 每页 12 项 × 0-3 量化**(替代 legacy 4-dim × 10):
...
**P2-13 multi-persona strict-eval**:
- brief.audience 是 `list[str]`(P2-13 multi-select)→ 对每个 persona 独立跑一遍完整 Step 1-3.5
...
**Step 3.5.2 image RAG 反查 alternative(P2-7)**:
...
**Step 3.6 feedback writeback(P3-5)**:
...
| `needs_theme_fix` | theme 层视觉(yaml 缺字段) | 主线程改 themes/*.yaml(P3-1 yaml 化后) |
...
status: ok
next_action: delivered | needs_author_rewrite | needs_visual_redo | needs_theme_fix | blocked_by_spot_check
deck_score_final: 8.5                 # P2-2 10 分制 · multi-persona 取 min(per_persona_scores)
verdict: excellent | good | needs_minor | needs_major | blocked_by_spot_check
...
# === P2-2 量化评分 ===
per_page_scores:                      # 每页 12 项 × 0-3
  - page: 1
    scores_12: {comprehension: 3, pacing: 2, wording: 3, visual: 2, ...}
    weighted: 8.7

# === P2-13 multi-persona ===
per_persona_scores:                   # 仅 list[str] audience 时填
  - {persona: cfo, deck_score_10: 8.5, weakest_dim: visual, weakest_pages: [5, 12]}
  - {persona: engineer, deck_score_10: 9.2, weakest_dim: density, weakest_pages: [8]}
blocking_persona: cfo                 # 决定 verdict 的 persona

# === Step 0.4 hot-reload(P2-4)===
hot_reload:
  used: true | false
  carried_over_chapters: [1, 2]
  rerun_chapters: [3]
...
needs_visual_redo_pages:              # triage = needs_visual_redo 时填(多类 triage 也填)
  - page: 8
    ...
    visual_query_match:               # P2-7 Step 3.5.2 image RAG 输出
      closest_template_page: ..., score: 0.78

# === P3-5 feedback ===
feedback_writeback:
  status: ok | skipped_no_plan | write_failed
  rows_written: 14
  rows_skipped: 0
  feedback_path: library/_rag/feedback.jsonl
...
**做的事**:
1. 复制用户提供的 .pptx → `library/pptx-templates/_source/<name>.pptx`(记 sha256 → meta.provenance.source_pptx_sha256 · P3-15)
...
3. **Step 2.6 watermark + 第三方品牌 LOGO detect**(P3-22 · `library/_rag/scripts/detect_watermark.py`):扫源 .pptx 媒体 + 渲染 PNG · 命中已知品牌词 / 半透明大图层 → 标 `needs_manual_review: true` + 强警告 user_review_drafts 阶段提示
...
| **模型** | **haiku-4-5**(P3-7 路由 · Opus 单价 19x Haiku 浪费) |
...
*why · 11 条贯穿系统的设计决定 · §4.7-4.11 为 P2-3/P3-7/P1-1/P3-9/P3-5 引入的新决策*
...
- critic Stage C(outline audit)+ Stage D(content audit)→ 合并为 stage=cd 单次合审(P2-3.2)
...
### 4.10 跨模板组合保 primary theme · P3-9
...
### 4.11 feedback loop opt-in · P3-5
...
- 总 wall clock ~40 min(vs P2-3 前 ~45 min · 减少冗余 critic 派发 ~5 min)

### 5.2 主线程派发表

*从 pipeline-protocol.md §1 速查*

完整表见 `.claude/pipeline-protocol.md` §1。关键派发触发(P2-3 7 步精简后):
...
| outline 用户批准 | Task(author, stage=D · **P2-3.2 后无中间 critic gate**) |
| content 用户批准 | Task(critic, **stage=cd · P2-3.2 合并合审**) |
...
| **extractor 写完 draft → 主线程错误恢复** | Task(iloveppt-self-check, 批量 targets) → 如 fail → Task(iloveppt-yaml-fixer) → 重跑 self-check 验证(P3-7)|
...
|---|---|---|---|---|---|---|---|---|
| **调用方式** | TeamCreate | Task | Task | Task | Task | Task | Task(P3-7 · 主线程错误恢复) | Task(P3-7 · 主线程错误恢复) |
...
| **读 outline.md** | — | ✓ | ✓ | ✓ | — | — | — | — |
| **读 content.md** | — | ✓ | ✓ | ✓ | **✗**(只读 PNG) | — | — | — |
| **读 render PNG** | ✓ Step 1 inspiration → image RAG(P2-8) | — | — | ✓(Step 3 QA + Step 4.3.5 image RAG)| ✓ | ✓(probe 8 PNG) | — | — |
| **写 brief.md** | ✓ 唯一 | — | — | — | — | — | — | — |
| **写 outline.md** | — | ✓ 唯一 | — | — | — | — | — | — |
| **写 content.md** | — | ✓ 唯一 | — | — | — | — | — | — |
| **写 deck_plan.json** | — | — | — | ✓ 唯一 + derived_from_sha256(P2-5)| — | — | — | — |
| **写 .pptx** | — | — | — | ✓ 唯一 | — | — | — | — |
| **写 report .md** | — | — | critic_report cd ✓(P2-3.2 合并) | visual_report ✓ | audience_review ✓ | — | normalized yaml report ✓ | fix log yaml ✓ |
| **写 `library/pptx-templates/items/<name>/...meta.yaml` draft** | — | — | — | — | — | ✓ 唯一(draft;用户审后入 DB)| — | — |
| **写 `library/_rag/feedback.jsonl`** | — | — | — | — | ✓ 唯一(P3-5 append-only · opt-in 影响) | — | — | — |
| **调 RAG(`library/search.sh`)** | ✓ Stage A 列模板 + Step 3.5 + Step 1 image RAG(P2-8) | ✓ Stage C/D + chapter-aware preferred-template(P3-9)| ✓ J5 | ✓ Step 2 注释 + Step 4.2.5 + Step 4.3.5 image RAG(P2-7) | ✓ Step 3.5 + Step 3.5.2 image RAG(P2-7)| — | — | — |
| **读 `library/pptx-templates/items/<name>/meta.yaml`** | ✓ Stage A 候选 | ✓ Stage D(每 chapter resolved theme) | — | ✓ build.py + 跨模板 tier1 deep-copy(P3-9) | — | — | — | — |
...
| **怎么扫敏感数据?** | pre-commit hook 自动跑(P3-19);手动:`library/_rag/scripts/redact.py --scan <file>` |
...
| **deck self-check fail 主线程怎么修?** | 派 `Task(iloveppt-self-check)` 跑 14 项 + 解析 → 派 `Task(iloveppt-yaml-fixer)` 修字面 → 重跑 self-check 验证(P3-7 · 详 §3.7)|
...
| [`docs/optimization/2026-05-27-iLovePPT-optimization-plan.md`](optimization/2026-05-27-iLovePPT-optimization-plan.md) | **v0.13.0 优化基准** — P0+P1+P2+P3 task list + baseline 度量对比 |
| [`docs/writing-custom-themes.md`](writing-custom-themes.md) | **自定义 theme 指南**(P3-1 yaml 化后)|
| [`docs/adding-new-layout.md`](adding-new-layout.md) | **加新 layout 指南**(P3-2 plugin auto-discover) |
| [`.claude/pipeline-protocol.md`](../.claude/pipeline-protocol.md) | **运行时协议** — Claude Code 主线程读的派发 / handoff / gate 权威活协议 |
| [`.claude/agents/critic-rubric.yaml`](../.claude/agents/critic-rubric.yaml) | **critic 21 项量化 SSOT**(P2-1 · evidence_requirement + severity_examples 权威定义) |
...
| [`library/deck-skeletons/README.md`](../library/deck-skeletons/README.md) | **6 个 deck skeleton**(P3-8) |
