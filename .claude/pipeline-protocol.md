...
> **流水线形态**: 6 agent + 1 旁路 · brainstorm → author → critic(C/D)→ builder → designer → audience(+ extractor 旁路)
...
iLovePPT 的 critic / designer / audience 三个评审类 agent 多轮迭代产物**全保留**,文件名带 `_r{N}` 后缀(round)。全保留为了事后追溯收敛轨迹("为什么 r1 fail r2 pass")。
...
| designer | `<working_dir>/designer/designer_report_r{N}.md`(N=1,2,...,5) |
...
```python
# 伪代码(critic / designer / audience 都遵守)
...
| designer | `prev_audience_review_path` | `<working_dir>/audience/audience_review_r{N-1}.md`(上一轮)|
...
2. **每个 teammate 独立窗口**:6 个 agent + 1 旁路按需各占一格,通过 `SendMessage` 跨窗口传递交付物路径(`brief.md` / `outline.md` / `content.md` / `deck_plan.json` / `.pptx` / report.md)。
...
主线程派 iloveppt-designer(auto trigger,不问用户)
  ...
  ↓ Step 6 写 designer_report_r{N}.md
  ↓
主线程派 audience(评新版本)
```
...
prev_audience_review_path: <working_dir>/audience/audience_review_r{N}.md  # 可选 · 第 2 轮起传
prev_designer_report_path: <working_dir>/designer/designer_report_r{N}.md  # 可选 · 第 2 轮起传
```

**历史反馈环**:第 2 轮起,主线程把上轮 `audience_review_r{N}.md` + `designer_report_r{N}.md` 路径传给 designer:
...
**designer 窗口每轮新建**(跟 critic / audience 一致) —— 无状态,所有 state 在 `designer_report_r{N}.md`。

**designer 不改 content.md**(原文是用户批准的 SSOT) —— 改的是 `deck_plan.json`(可选地用 builder 的 `.postbuild.md` 副本)。

## 9. builder → audience handoff + 反馈环

**串行 + 9 分硬阈值** — builder 完成 + **designer 视觉优化完**后再派 audience。audience `overall_score < 9` 一律不交付,自动进入修复环,**直到拿到 ≥ 9 才允许交付**。
...
├── designer/
│   ├── designer_report_r{N}.md                    ← 最后一轮 designer 视觉优化报告
...
| critic Stage D pass / pass_with_notes,首次完整 build | 派 **iloveppt** (builder) + 自动接 **iloveppt-designer** | builder 跑 Step 0 + 机械视觉 QA;designer 自动接 library-first 视觉优化(详见 §13.6 / §13.7) |
| **trivial rebuild**(content < 10% 改 / 仅 quick fix · §13.4) | **主线程自跑** `build.py` + 验证改动页 | **不派** builder / designer · 节省一轮 dispatch overhead |
| audience 反馈含 needs_designer_revision · 无 author/theme 改 | 派 **iloveppt-designer**(直接 · 跳 builder)| designer 自跑 rebuild,builder 工作冗余 |
| audience 反馈仅 needs_author_rewrite | 派 **iloveppt-author** → trivial rebuild(§13.4)→ audience | 跳过 designer 整轮 |
...
**反例 3**(本次 run 教训):builder + designer 每轮分两次 dispatch · 4 轮 = 8 dispatch。**应该按 §13.7 链式调用规则,trivial rebuild 跳过两者,只在真需要时分派**。

**例外**:如果用户明确说"你自己改"/"不用派 agent",尊重决定。

**关于 builder / designer 物理合并**:本协议**不合并**两个 agent 文件(各自人设 + 工具集 + 红线相对独立,合并会增加单文件复杂度且破坏现有所有引用)。**§13.7 在 dispatch 协议层实现"主线程视角合一"** —— trivial 改跳过 / 只 needs_designer_revision 跳 builder / 只 needs_author_rewrite 跳 designer —— 实际效率收益跟物理合并相当,无破坏性。
...
| 工具维度 | 检查问题 |
|---|---|
| `library/visual-patterns/` | designer 上轮 Read INDEX.md 了吗?有哪些 pattern 命中本 deck 但还没用? |
...
| "同质化" / "审美疲劳" / "信息平铺" / "卡片堆" | 派 designer + 强制要求 Read INDEX.md | "library/visual-patterns/INDEX.md, RAG search 当前 deck 弱点 page 的 content intent, 推荐 2-3 个 layout swap, 返给主线程评估是否实现 make_ 函数" |
| "字号小" / "label 难辨" / "diagram 模糊" | 派 designer + 重画 diagram | "用 matplotlib / drawio 重生成 chart, label 字号 ≥ 14pt (2400×1500 base size)" |
| "BCG / 稳重感 / 卡通 / hero 数字" | 派 designer 改 deck_plan.json + 视觉层 | "cover / closing visual,换 hero 数字 anchor 或换 layout,不动 theme 代码" |
...
### 13.6 designer dispatch · library-first 协议

派 designer 前,主线程**入参必须包含** `library_lookup_required: true` + 提示用 `${CLAUDE_PROJECT_DIR}/library/visual-patterns/INDEX.md` 或 RAG `search.sh` 查匹配 pattern:

```yaml
working_dir: ...
pptx_path: ...
library_lookup_required: true   # designer 必须先 Read INDEX.md
library_lookup_focus: ["audience.weakest_pages", "audience.same_layout_repeated"]
# designer 必须在 designer_report_r{N}.md 列:
#   - patterns_searched: [...]
#   - patterns_matched: [...]
#   - patterns_applied / patterns_rejected_with_reason: [...]
```

**designer 不查 library = designer dispatch 视为部分完成**,主线程下轮 dispatch 时**强制要求**先查 library。

**反例**:R1-R4 designer 共 4 次 dispatch,**0 次 Read library/visual-patterns/INDEX.md**。R5 用户喊才挖出来,顶 +0.2 deck 收益。

### 13.7 builder + designer 在 trivial 改动时按"renderer 模式"链式调用

builder + designer 是顺序耦合的(builder 出 .pptx → designer 改 deck_plan.json → 重 build → audience 评)。**主线程在以下场景视为合一 dispatch**(物理上仍两个 agent 文件,但主线程视角合并):

| 场景 | 主线程动作 |
|---|---|
| 完整 build(首次 / 跨版本) | 派 builder + 自动派 designer(designer 入参带 library_lookup_required) · 串行不 ack 等用户 |
| trivial rebuild(§13.4 条件)| 主线程**自跑** build.py,**不派**任何 agent · 直接进 audience(若是 audience 修复循环) |
| audience 反馈仅 needs_designer_revision(无 author / theme 改) | 派 designer · 跳过 builder(designer 自己 rebuild) |
| audience 反馈仅 needs_author_rewrite | 派 author 改 content → trivial rebuild(§13.4)→ audience · 跳过 designer |

**反例**:R1-R4 每轮都派 builder + 派 designer + audience,**4 轮 × 3 agent = 12 dispatch**。按 13.7 应该:R1 完整 build(3 dispatch)+ R2-R4 trivial rebuild(1 dispatch / 轮)+ designer 只在 needs_designer_revision 时派 = **6-7 dispatch 总数**(节省 ~40%)。
