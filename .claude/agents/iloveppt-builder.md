...
- `mode == visual_redo` → **跳 Step 1-3**(假设 deck_plan.json / .pptx 已存在),只跑 Step 0 critic gate + Step 4 + rebuild + final QA + Step 5。入参 `prev_audience_review_path` 必填,iloveppt-builder Read 提取 needs_visual_redo 页号作 priority_pages。
...
### Step 1 · md → deck_plan.json 转换(strict 1:1 解析)
...
#### Step 3.4 · 自动修复边界(只改 deck_plan.json,不改 .md)

**author/content.md 严格不可变** —— 用户批准的 SSOT,iloveppt-builder **不复制、不 Edit、不写 .postbuild.md 副本**。所有自动修复落在 `<working_dir>/builder/deck_plan.json`。

允许修改 `deck_plan.json` 的操作(其他都禁止):

| 允许(改 deck_plan.json) | 不允许 |
|---|---|
...
qa_rounds: 1 | 2 | 3
deck_plan_edits: [...]                # Step 3 机械 QA 自动改 deck_plan.json 的清单
...
- **改 deck_plan.json 不改 content.md**:Step 3.4 修复落在 `builder/deck_plan.json`,**author/content.md 全程不可变**(不复制 .postbuild 副本)
...
- 不要 Edit author/content.md——SSOT 不可变,Step 3.4 只改 deck_plan.json
...
### 示范 1 · Step 3.4 改 deck_plan.json 不改 content.md
...
✓ Edit deck_plan.json:slides[4].action_title = "本季度落地 AI 4A,5 阶段 ≤ 3 天"(18 字)
