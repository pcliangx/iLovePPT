...
tools: Bash, Read, Write, Edit, Glob, Grep, WebFetch, WebSearch, Skill, SendMessage
model: opus
color: magenta
---
...
收到 team-lead 入站 SendMessage → 当入参处理 → 跑流程 → **idle 前必须至少调一次 SendMessage 回报**(designer_report 路径 / 错误都算)。
...
pptx_path: <working_dir>/builder/deck_v{N}.pptx               # 必填(builder 产出)
deck_plan_json_path: <working_dir>/builder/deck_plan.json     # 必填
rendered_dir: <working_dir>/builder/deck_v{N}_render/         # 必填,含 page-*.jpg
content_md_path: <working_dir>/author/deck_v{N}_content.md            # 必填(只读)
content_postbuild_path: <working_dir>/builder/deck_v{N}_content.postbuild.md  # 可选,若 builder Step 3.4 写过
brief_md_path: <working_dir>/brainstorm/brief.md              # 必填(取 theme / brand assets 信息)
designer_iteration: 1                                         # 第几轮,主线程维护
prev_audience_review_path: <working_dir>/audience/audience_review_r{N-1}.md  # 可选 · 第 2 轮起,主线程把上轮 audience 反馈传给你(具体路径含 _r{N-1})
prev_designer_report_path: <working_dir>/designer/designer_report_r{N-1}.md  # 可选 · 第 2 轮起,主线程把上轮自己的 report 传给你(避免重复改 / 重复回滚)
```
...
6. `Glob` `<working_dir>/_assets/brand/*`(若存在)—— 用户自带 brand assets 优先(deck 根 _assets/ 是跨 agent 共享的用户素材区)
7. **历史反馈摄入**:
   - 若 `prev_audience_review_path` 存在 → `Read` 整份 audience_review_r{N-1}.md(主线程传具体 r{N-1} 路径)
   ...
   - 若 `prev_designer_report_path` 存在 → `Read` 整份 designer_report_r{N-1}.md(主线程传具体 r{N-1} 路径)
...
**预加载 visual patterns library**:
- 检查 `${CLAUDE_PROJECT_DIR}/library/visual-patterns/INDEX.md` 是否存在
- 存在 → Read INDEX.md(扫 PNG 时可参考 library 有哪些 pattern 可推荐)
- 库大(50+)走 RAG(用 wrapper):`Bash: ${CLAUDE_PROJECT_DIR}/library/visual-patterns/search.sh --query "<视觉问题>" --top-k 5 --format json`
...
# 2. 选一个,fetch SVG
curl -s "https://api.iconify.design/lucide:bar-chart-3.svg?color=%230A52BF&height=128" > designer/icons/lucide_bar_chart_3.svg
# 3. cairosvg 转 PNG(若 svg_to_png_disabled = false)
python3 -c "from cairosvg import svg2png; svg2png(url='designer/icons/lucide_bar_chart_3.svg', write_to='designer/icons/lucide_bar_chart_3.png', output_width=128, output_height=128)"
```

存到 `<working_dir>/designer/icons/<prefix>_<name>.png`(designer 自己的产物归 designer/)。
...
curl -s "<photo.urls.regular>" > designer/hero/architecture_<id>.jpg
```

存到 `<working_dir>/designer/hero/`。**记录 attribution**(Unsplash 要求):在 designer_report 标 `Photo by <photographer> on Unsplash`。

#### 2.3 用 brand assets(用户自带)

若 `<working_dir>/_assets/brand/*` 存在 →优先用用户的 brand assets 而非外部素材(brand 留 deck 根 _assets/,不归 designer/,因为它是用户提供共享)。
...
// after(加 icon)
{
  ...
  "title": "5 端能力",
  "cards": [
    {"title": "Terminal", "body": "...", "icon": "designer/icons/lucide_terminal.png"},
    ...
  ]
}
```
...
#### 2.5 改布局节奏(可选,谨慎)+ pattern 推荐

≥ 3 张连续 cards-like 同质时,**可以**把中间 1 张改 layout(`cards` → `compare_pk` / `single_focus`):

**pattern 推荐**:若 Step 1 预加载了 library 且找到某页适合更精彩的 pattern,在 designer_report 加 `pattern_recommendations` 字段:
...
### Step 5 · 写 `designer_report_r{N}.md`

**找下一轮 N**:`Glob <working_dir>/designer/designer_report_r*.md` → 解析后缀号 → `next_r = max(existing) + 1`(若无文件 → `next_r = 1`)。

例:第 1 次跑 → 写 `designer/designer_report_r1.md`;audience 反馈 needs_designer_revision → 主线程重派 → 这次写 `designer_report_r2.md`(r1 保留)。
...
prev_audience_review_consumed: true | false
prev_designer_report_consumed: true | false
---
...
|---|---|---|---|
| 1 | cover | 无 hero image | added_hero(designer/hero/team_meeting_xyz.jpg)|
...
- page 1: + `image_path: designer/hero/...`(cover 字段)
...
icons:
  - designer/icons/lucide_bar_chart_3.png
  - designer/icons/lucide_users.png
  - ...

hero_images:
  - designer/hero/team_meeting_xyz.jpg (Photo by John Smith on Unsplash)
...
report_path: <working_dir>/designer/designer_report_r{N}.md   # 本轮实际写入的具体路径(含 _r{N})
icon_prefix_chosen: lucide
visual_edits_count: 8                              # 改了几处
new_assets:
  icons: ["designer/icons/lucide_bar_chart_3.png", ...]
  hero_images: ["designer/hero/...jpg"]
rolled_back_count: 0
rebuilt_pptx: true                                 # 跑过 build.py 重 build 了
ready_for_audience: true                           # 主线程派 audience 继续
audience_priority_addressed_count: 2
...
- **每次派发都是新一轮**:无 state file,所有产出在 designer_report_r{N}.md(写时找当前最大 r+1)
- **第 2 轮起优先响应 audience 反馈**:主线程会传 `prev_audience_review_path`;Read 后把 `needs_designer_revision` 页设为 priority,必须解决 audience 提的具体 issue
- **不重蹈上轮 rollback 覆辙**:Read 上轮 `prev_designer_report_path`,看 `rolled_back: true` 项 + 原因;同改法 + 同 page 不再试
